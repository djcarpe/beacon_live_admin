  ...
  def render(assigns) do
    ...
    <div>
      ...
      <.header>
        <%= @page_title %>
        <:actions>
          <.button phx-disable-with="Saving..." form="resource-links-form" class="btn-primary">Save Changes</.button>
        </:actions>
      </.header>

      <.main_content>
        <div class="flex gap-4">
          <.button type="button" phx-click="add" class="btn-primary">New Resource Link</.button>
          <.button type="button" phx-click="show-new-attribute-modal" class="btn-primary">New Attribute</.button>
        </div>
        ...
        <.modal :if={@show_modal} id="new-attribute-modal" show={true} on_cancel={JS.push("hide-new-attribute-modal")}>
          ...
          <.simple_form :let={f} for={%{}} as={:attribute} phx-submit="save-new-attribute">
            ...
            <div>
              ...
              <.button class="btn-primary mt-2">Add custom attribute</.button>
            </div>
          </.simple_form>
        </.modal>
      </.main_content>
