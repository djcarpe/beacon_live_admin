        ...
        <%= if @schema do %>
          <div class="mb-4">
            <input type="text" name="search" value={@search} phx-keyup="search" phx-debounce="300"
              ...
              class="w-full px-3 py-2 border border-base-300 rounded-lg bg-base-100 text-base-content text-sm" />
          </div>

          <div class="flex gap-2 mb-4">
            <button type="button" phx-click="set_tab" phx-value-tab="queries"
              class={"px-4 py-2 text-sm rounded-lg transition-colors #{if @active_tab == "queries", do: "bg-primary text-primary-content", else: "bg-base-200 text-base-content/80"}"}>
              Queries (<%= length(@schema["queries"] || []) %>)
            </button>
            <button type="button" phx-click="set_tab" phx-value-tab="mutations"
              class={"px-4 py-2 text-sm rounded-lg transition-colors #{if @active_tab == "mutations", do: "bg-primary text-primary-content", else: "bg-base-200 text-base-content/80"}"}>
              Mutations (<%= length(@schema["mutations"] || []) %>)
            </button>
            <button type="button" phx-click="set_tab" phx-value-tab="types"
              class={"px-4 py-2 text-sm rounded-lg transition-colors #{if @active_tab == "types", do: "bg-primary text-primary-content", else: "bg-base-200 text-base-content/80"}"}>
          ...
          <div class="mt-1 space-y-1">
            <%= for arg <- @operation["args"] do %>
              <div class="flex items-center gap-2 text-xs">
                ...
                <span :if={arg["defaultValue"]} class="text-base-content/50">= <%= arg["defaultValue"] %></span>
              </div>
            <% end %>
          </div>
        </div>
  ...
  defp type_card(assigns) do
    ...
    <div class="p-4 bg-base-100 border border-base-300 rounded-lg">
      <div class="flex items-center gap-2">
        <span class="px-2 py-0.5 text-xs rounded bg-base-200 text-base-content/70"><%= @type["kind"] %></span>
      ...
      <%= if @type["fields"] && @type["fields"] != [] do %>
        <div class="mt-2 border-t border-base-300 pt-2 space-y-1">
        ...
        <div class="mt-2 border-t border-base-300 pt-2">
          ...
          <div class="flex flex-wrap gap-1 mt-1">
            <%= for val <- @type["enumValues"] do %>
              <span class="px-2 py-0.5 text-xs font-mono bg-warning/10 text-warning rounded"><%= val["name"] %></span>
            <% end %>
          </div>
        </div>
      <% end %>
