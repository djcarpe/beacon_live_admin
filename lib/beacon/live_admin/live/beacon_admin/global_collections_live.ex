defmodule Beacon.LiveAdmin.BeaconAdmin.GlobalCollectionsLive do
  @moduledoc false
  ...
  @impl true
  def menu_link("/beacon", :index), do: {:submenu, "Global Collections"}
  ...
  def handle_params(_params, _url, socket) do
    ...
    {:noreply,
     ...
     |> assign(page_title: "Global Collections")}
  end
    ...
    case result do
      {:ok, _} ->
        ...
        {:noreply,
         ...
         |> put_flash(:info, "Global collection saved")}
  ...
  def handle_event("delete", %{"id" => id}, socket) do
    ...
    case Content.delete_collection(site, tt) do
      {:ok, _} ->
        ...
        {:noreply,
         socket
         |> assign(collections: collections, confirm_delete: nil)
         |> put_flash(:info, "Global collection deleted")}
  ...
  def render(assigns) do
    ...
    <.header>
      Global Collections
      <:actions>
        <.button phx-click="new" class="btn-primary">Create New Global Collection</.button>
      </:actions>
    </.header>

    <p class="text-sm text-base-content/60 -mt-4 mb-4">
      Collections available to all sites (site = nil)
    </p>

    <.main_content :if={@show_form} class="mb-6">
      <div class="px-2 py-4">
        <h2 class="text-base font-semibold text-base-content mb-4">
          <%= if @editing, do: "Edit Global Collection", else: "New Global Collection" %>
        </h2>
    ...
    <.main_content>
      <.table id="global-collections" rows={@collections} row_id={&"gtt-#{&1.id}"}>
