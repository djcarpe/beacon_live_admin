    ...
    <div>
      ...
      <.main_content>
        <.modal :if={@show_nav_modal} id="confirm-nav" on_cancel={JS.push("stay_here")} show>
          ...
          <.button type="button" class="btn-ghost" phx-click="stay_here">
            Stay here
          </.button>
          <.button type="button" class="btn-error" phx-click="discard_changes">
            Discard changes
          </.button>
        </.modal>
        ...
        <.modal :if={@show_delete_modal} id="confirm-delete" on_cancel={JS.push("delete_cancel")} show>
          ...
          <div class="flex justify-end w-full gap-4 mt-10">
            <.button type="button" class="btn-error" phx-click="delete_confirm">
              Delete
            </.button>
            <.button type="button" class="btn-ghost" phx-click="delete_cancel">
              Cancel
            </.button>
          </div>
        </.modal>
        ...
        <div class="grid items-start grid-cols-1 grid-rows-1 mx-auto gap-x-8 gap-y-8 lg:mx-0 lg:max-w-none lg:grid-cols-3">
          ...
          <div :if={@form} class="w-full col-span-2">
            <.form :let={f} for={@form} class="flex items-end gap-4" phx-change="validate" phx-submit="save_changes">
              ...
              <.button type="button" class="btn-error" phx-click="delete">Delete</.button>
            </.form>
            ...
            <.button class="btn-neutral mt-4" phx-click={JS.navigate(beacon_live_admin_path(@socket, @beacon_page.site, "/components/#{@component.id}/slots/#{@selected.id}/attrs/new"))}>
              Add new Attribute
            </.button>
          </div>
        </div>
